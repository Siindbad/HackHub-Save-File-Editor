...
def parse_suggestion_before_after(message: Any) -> Any:
    before = None
    after = None
...
def build_overlay_suggestion_payload(has_overlay: Any, message: Any, line_no: Any) -> Any:
    if not has_overlay:
        return None
    msg = str(message or "")
    if not msg:
...
def is_symbol_error_note(note: Any) -> Any:
    return str(note or "").startswith("symbol_") or str(note or "") in error_symbol_notes()


def error_marker_colors(note: Any, palette: Any, insertion_only: Any=False) -> Any:
...
def current_error_palette(variant: Any, theme: Any) -> Any:
...
def selection_colors(theme: Any, use_error_palette: Any=False, error_palette: Any=None) -> Any:
