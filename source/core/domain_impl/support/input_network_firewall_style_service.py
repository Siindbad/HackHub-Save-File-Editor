...
def render_firewall_input_rows(owner: Any, host: Any, normalized_path: Any, row_defs: Any) -> Any:
    ...
    def _identity_field(parent: Any, text: str, fg: str) -> tk.Entry:
        var = tk.StringVar(value=str(text))
        entry = tk.Entry(
            parent,
            textvariable=var,
            relief="flat",
    ...
    for row in row_defs:
        ...
        top = tk.Entry(
            left,
            textvariable=tk.StringVar(value=str(row.get("display_id", ""))),
            relief="flat",
            bd=0,
            highlightthickness=0,
            readonlybackground=left_bg,
        ...
        bind_input_widget = getattr(owner, "_bind_input_context_widget", None)
        if callable(bind_input_widget):
            bind_input_widget(top, allow_paste=False)
        top.pack(anchor="w", padx=7, pady=(4, 1))
        _identity_field(left, f"IP : {row.get('ip', '')}", meta_fg).pack(anchor="w", padx=7)
        _identity_field(left, f"LAN : {row.get('lan_ip', '')}", meta_fg).pack(anchor="w", padx=7)
        user_value = str(row.get("user", "") or "").strip() or "N/A"
        pass_value = str(row.get("password", "") or "").strip() or "N/A"
        _identity_field(
            left,
            f"User : {user_value}",
            meta_fg if user_value != "N/A" else na_fg,
        ).pack(anchor="w", padx=7)
        _identity_field(
            left,
            f"Pass : {pass_value}",
            meta_fg if pass_value != "N/A" else na_fg,
...
def _render_field_input(
    ...
    bind_input_widget = getattr(owner, "_bind_input_context_widget", None)
    if callable(bind_input_widget):
        bind_input_widget(entry, allow_paste=bool(is_editable))
    if not is_editable:
        return
