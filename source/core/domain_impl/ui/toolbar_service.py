...
def update_find_entry_layout(owner: Any, *, tk_module: Any, expected_errors: Any) -> None:
    host = getattr(owner, "_find_entry_host", None)
    ...
    if variant == "KAMUE" and style == "A":
        try:
            ...
            owner.find_entry.pack_configure(fill="x", expand=True, padx=(2, 2), pady=0, ipady=2)
        except expected_errors:
            return
        return
    if style == "A":
        try:
            slot = getattr(owner, "_find_entry_slot", None)
            if slot and slot.winfo_exists():
                slot.place_forget()
            edge = getattr(owner, "_find_entry_edge_line", None)
            if edge and edge.winfo_exists():
                edge.place_forget()
            inner_edge = getattr(owner, "_find_entry_inner_edge_line", None)
            if inner_edge and inner_edge.winfo_exists():
                inner_edge.place_forget()
            owner.find_entry.place_forget()
            ...
            owner.find_entry.pack_configure(
                fill="none",
                expand=False,
                padx=0,
                pady=(5, 3),
    ...
    if style == "B":
        ...
        host_width = int(owner._find_entry_target_width())
        host_height = int(search_spec.get("height", 32) or 32)
        host_height = min(host_height, owner._siindbad_b_button_height("find", default_height=33))
        ...
        try:
            ...
            if squeezed:
                if edge is None or not edge.winfo_exists() or edge.master is not host:
                    edge = tk_module.Frame(host, bg=palette.get("border", "#349fc7"), bd=0, highlightthickness=0)
                    owner._find_entry_edge_line = edge
            ...
            if isinstance(input_box, (tuple, list)) and len(input_box) == 4:
                ...
                if squeezed:
                    if inner_edge is None or not inner_edge.winfo_exists() or inner_edge.master is not host:
                        inner_edge = tk_module.Frame(
                            host,
                            bg=palette.get("border_active", "#a9ddf0"),
                            bd=0,
                            highlightthickness=0,
                        )
                        owner._find_entry_inner_edge_line = inner_edge
                    inner_edge.place(
            ...
            return
        return
    width_px = owner._find_entry_target_width()
    try:
        slot = getattr(owner, "_find_entry_slot", None)
        if slot and slot.winfo_exists():
            slot.place_forget()
        ...
        owner.find_entry.pack_configure(fill="x", expand=True, padx=(2, 2), pady=0, ipady=1)
    except expected_errors:
        return


def apply_toolbar_layout_max(
    owner: Any,
    center: Any,
    host: Any,
    *,
    expected_errors: Any,
    compute_centered_toolbar_position: Any,
) -> None:
    ...
    logo_widget = getattr(owner, "logo_frame", None) or getattr(owner, "logo_label", None)
    ...
    placement = compute_centered_toolbar_position(
        toolbar_w=toolbar_w,
        toolbar_h=toolbar_h,
        host_w=host_w,
        host_h=host_h,
...
def _build_toolbar_structure(owner: Any, top, inter_button_pad):
        ...
        owner.find_entry.bind("<Button-3>", owner._show_find_entry_context_menu, add="+")
        owner.find_entry.bind("<Shift-F10>", owner._show_find_entry_context_menu, add="+")
        owner.find_entry.bind("<Menu>", owner._show_find_entry_context_menu, add="+")
